...
export function Section({
  ...
  return (
    <section id={id} className={cn('relative', SURFACE[surface])}>
      ...
      <div className={cn('relative mx-auto w-full max-w-5xl px-6 py-16', className)}>{children}</div>
    </section>
  )
}
...
export function SectionHeading({ children }: { children: ReactNode }) {
  ...
  return (
    <h2 className="font-display text-4xl font-bold tracking-[-0.03em] sm:text-[48px] sm:leading-[1.1]">
      {children}
    </h2>
  )
}
