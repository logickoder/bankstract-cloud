...
`parse` accepts `Uint8Array | ArrayBuffer | Blob`. See [Authentication](/docs/authentication) for keys and [the parse endpoint](/docs/api/parse_v1_parse_post) for the response shape.
...
Every non-2xx response throws a typed `BankstractError` subclass. The API key never appears in the error. The thrown error mirrors the [error envelope](/docs/errors): `status`, `errorClass`, and `markerCoverage` where relevant.
