...
3. **Billing.** Set `PAYSTACK_SECRET_KEY` + the `PAYSTACK_PLAN_*` codes for NGN subscription billing; empty = free self-host (no charges). The worker holds the secret and verifies webhooks with HMAC-SHA512.
